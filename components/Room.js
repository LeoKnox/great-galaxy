...
export default function Room({ showMenu, setShowMenu }) {
  const [characterPosition, setCharacterPosition] = useState({
    position: [-0.5, 0, -1.5],
  });
  const stairsLoc = [import Stairs from "./Stairs";
import PlayerInfo from "./PlayerInfo";
import Character from "./Character";
import Floor from "./Floor";
import Walls from "./Walls";
...
export default function Room({ showMenu, setShowMenu }) {
  ...
  function collision(x = -2.5, y = 0, z = -2.5, offsetx, offsety, offsetz) {
    ...
    if (c) {
      setShowMenu(true);
      setShowMenu(showMenu.push(<button>Red</button>))
  ...
  function collision(x = -2.5, y = 0, z = -2.5, offsetx, offsety, offsetz) {
    ...
    if (c) {
      setShowMenu(true);
      setShowMenu([...showMenu, <button>Red</button>])
      return [x + offsetx, offsety + 1, z + offsetz];
    }
    setShowMenu(false);
    return [x + offsetx, offsety, z + offsetz];
